...
export default function AdvancedReportsPage() {
  ...
  const materialUsage = [
    { name: "PLA Preto", used: 8.5, max: 10, color: "bg-blue-600" },
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 mt-2">
         <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600 font-bold">BI, Analytics e classificação profunda do catálogo de produtos vendidos.</p>
            </div>
         </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
         <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-xl relative overflow-hidden group">
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
               ...
               <div className="flex items-center gap-1 text-[9px] font-black text-blue-600 bg-blue-600/10 px-2 py-0.5 rounded uppercase tracking-wider">
         ...
         <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-xl relative overflow-hidden group">
            ...
            <h3 className="text-3xl font-black text-white font-mono tracking-tighter relative z-10">
              {totalItemsSold} <span className="text-xs text-gray-600 font-bold">und</span>
            </h3>
         ...
         <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-xl relative overflow-hidden group">
            ...
            <h3 className="text-3xl font-black text-white font-mono tracking-tighter relative z-10">
              {totalHistoricSold} <span className="text-xs text-gray-600 font-bold">und</span>
            </h3>
         ...
         <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-xl relative overflow-hidden group">
            ...
            <h3 className="text-3xl font-black text-white font-mono tracking-tighter relative z-10">
              {totalCurrentSold} <span className="text-xs text-gray-600 font-bold">und</span>
            </h3>
            <p className="text-[10px] text-gray-600 font-bold mt-2 relative z-10">Vendas que deram baixa no inventário</p>
         </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-[2.5rem] p-8 shadow-2xl space-y-6">
         <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-gray-200 pb-6">
            <div>
               ...
               <p className="text-xs text-gray-600 font-bold mt-1">Classificação completa baseada nas vendas finalizadas do sistema.</p>
            </div>

            {/* BARRA DE PESQUISA */}
            <div className="relative max-w-sm w-full">
               <Search className="h-4 w-4 text-gray-600 absolute left-4 top-1/2 -translate-y-1/2" />
               <input 
                 type="text" 
         ...
         {loading ? (
           <div className="flex flex-col items-center justify-center py-20 gap-3">
              ...
              <p className="text-xs font-black text-gray-600 uppercase tracking-widest">Computando dados de vendas...</p>
           </div>
         ) : filteredRanking.length === 0 ? (
           <div className="text-center py-20 space-y-3">
              <AlertTriangle className="h-10 w-10 text-amber-500 mx-auto" />
              <h4 className="text-sm font-black text-white uppercase">Nenhum produto vendido encontrado</h4>
              <p className="text-xs text-gray-600 font-bold">Faça a importação de planilhas de vendas finalizadas para popular o ranking.</p>
           </div>
         ) : (
                   ...
                   >
                      ...
                      <div className="flex items-center gap-5">
                         ...
                         <div className="space-y-1">
                            <h4 className="text-sm font-black text-white group-hover:text-blue-600 transition-colors uppercase">{item.name}</h4>
                            <div className="flex flex-wrap items-center gap-3 text-[10px] text-gray-600 font-bold uppercase tracking-widest">
                               <span>SKU: <span className="font-mono text-slate-300">{item.sku}</span></span>
                               <span>•</span>
                               <span className="text-blue-600 font-mono">R$ {item.revenue.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} faturados</span>
                            </div>
                         </div>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-8">
                         ...
                         <div className="text-right min-w-[80px]">
                            <p className="text-[9px] font-black uppercase tracking-widest text-gray-600 mb-0.5">Total Vendido</p>
                            <p className="text-2xl font-black text-white font-mono">{item.quantity} <span className="text-[10px] text-gray-600 font-bold uppercase tracking-normal">und</span></p>
                         </div>
                      </div>
         ...
         <div className="bg-white border border-gray-200 rounded-3xl p-8 shadow-xl">
               ...
               {weeklyProduction.map((data, idx) => (
                 <div key={idx} className="flex-1 flex flex-col items-center justify-end gap-3 group relative cursor-pointer">
                    ...
                    <div className="w-full max-w-[40px] flex flex-col-reverse justify-start relative">
                       {/* Success Bar */}
                       <div 
                         className="w-full bg-blue-600 rounded-b-md rounded-t-sm transition-all duration-1000 ease-out shadow-[0_0_15px_rgba(34,211,238,0.2)]" 
                     ...
                     <span className="text-[10px] font-black text-gray-600 uppercase tracking-widest">{data.day}</span>
                 </div>
               ))}
            </div>
            ...
            <div className="flex items-center justify-center gap-6 mt-8 pt-6 border-t border-gray-200">
               ...
               <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Falha</span>
               </div>
            </div>
         </div>
                   ...
                   <div key={idx} className="space-y-2">
                      <div className="flex justify-between items-end">
                         <span className="text-[12px] font-bold text-white flex items-center gap-2">
                            <Box className="h-3.5 w-3.5 text-gray-600" /> {mat.name}
                         </span>
               ...
               <div key={idx} className="p-6 flex items-center justify-between hover:bg-white/5 transition-colors">
                  ...
                  <div className="flex items-center gap-10">
                     <div className="text-right">
                        <p className="text-[10px] font-black uppercase tracking-widest text-gray-600 mb-1">Horas Totais</p>
                        <p className="text-lg font-black text-white font-mono">{printer.hours}h</p>
                     </div>
                     <div className="text-right">
                        <p className="text-[10px] font-black uppercase tracking-widest text-gray-600 mb-1">Eficiência (OEE)</p>
                        <p className={cn(
